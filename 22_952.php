<?php include 'head.php'; ?>
<title>Berg Furniture | Captain's Bed | 22&ndash;952</title>

</head>
	<body>
    	<div id="outside_container">
    		
			<?php include 'options.php'; ?>
			
            <div id="tip"></div>
            <div class="container">
                
                <?php include 'header.php'; ?>
                
               <div class="prodContainer">
               		<div class="prodTitle"><style> .prodTitle p a:hover {color:#06dec7 ;}</style>
                    	<p><a href="jr_captains_bed.php">The Jr. Captain's Bed Collection</a></p>
                    </div>
                 <div class="prodMain">
                        <div class="prodCont">
                        		<h2>Low Frame Full w/16 Drawers</h2><h3>#22-952</h3>
                                <br />
                                <p style="font-size:14px;">
                                The Captain's motto:  less is more... but more is better!  This Jr. Captain's bed features more storage space per square inch than any other bed out there. It has 8 extra-deep drawers on each side (for a total of 16), 6 cubby-holes, and a Bookcase Headboard with flourescent lighting! And it has the build quality and style to make any captain jealous! 
                                </p>
                                <br />
                                <br />
                                <h3>Features</h3>
                                <ul>
                                	<li>Full-Size Bed</li>
                                    <li>8 Drawers on each side</li>
                                    <li>Side Shelves</li>
                                   
                                </ul>
                                <div class="dimensions"><img src="img/dimbkg.png"/>
                                    <div style="top:21px; left: 5px;">27"H</div>
                                    <div style="top:40px; left: 40px;">58"D</div>
                                    <div style="top:77px; left: 70px;">77"L</div>
                                </div>
                                <br />
                               <a href="#" rel="#knobs">Click for Knob Options&nbsp;&gt;&gt;</a><br />
                                <a href="#" rel="#finishes">Click for Finish Options&nbsp;&gt;&gt;</a></br >
								<a href="#" rel="#accents">Click for Accent Options&nbsp;&gt;&gt;</a>
                        </div>
                   <div class="prodShow">
                   	 <div class="imgWrap">
                            	<img src="img/1px.gif" width="500" height="392" />
                                <div id="image_caption"></div>
                            </div>
                            <a class="prevPage browse left"></a>
                            	<div class="scrollable">
                                	<div class="items">
                                    	<img src="img/MainProdPics/22-952B_t.jpg" 
                                        title="Shown in Nutmeg with Antique Copper Knobs; <b><i>Optional</b> Bookcase Headboard (22-95B) and Hanging Shelf</i>"/>
                                        
                                        <img src="img/MainProdPics/22-952A_t.jpg" 
                                        title="Shown in Nutmeg with Antique Copper Knobs; <b><i>Optional</b> Bookcase Headboard (22-95B)</i>"/>
                                        
                                        <img src="img/MainProdPics/22-952C_t.jpg" 
                                        title="The <u>Optional Bookcase Headboard and Hanging Shelf</u> complete the set and add more shelf space!"/>
                                       
                                       
                                    </div>
                                </div>
                            <a class="nextPage browse right"></a>
                     </div>
                         
                      <div id="iconBar">
                            
                         <div id="icons">
                           	 <img src="img/images/icons/1.png" 
                                	title="<h4>One Bed</h4><br />This Smart Bedroom System is designed to provide an enriching environment for one child" />
                             <img src="img/images/icons/stairs.png" 
                                	title="<h4>Berg's Signature Staircase</h4><br />This system is equipped with Berg's Signature Staircase. This innovative 				 											original feature provides safe access to the top loft and lots of storage in each step" />
                             <img src="img/images/icons/twin.png" 
                                title="<h4>Twin Bed</h4><br />The frame on this furniture set is built to support a twin 	
                                			 sized bed. Mattress not included." />
                                             <div id="rollover">Roll-over icons for more info</div>
                             
                         </div>
                     </div>        
                   </div>
                    <div class="prodFtr">
                    	<style> #thisColl a:hover {color:#8be93b;}</style>
                        <div id="thisColl"><b><a href="/jr_captains_bed.php">All Jr. Captain's Beds</a></b>
                        		<br /><br />
                              <ul>
                               	  <li><a href="22_940.php"><b>22-940</b> High Frame Twin</a></li>
                                  <li><a href="22_950.php"><b>22-950</b> Low Frame Twin</a></li>
                                  <li style="display:block; background-color: #8be93b;color:#000;"><b>22-952</b> Low Frame Full 16 Drawer</li>
                                  <li><a href="22_959.php"><b>22-959</b> Low Frame Full</a></li>
                                  <li><a href="22_965.php"><b>22-965</b> High Frame Twin w/Stairs</a></li>
                                  
                                  <br />
                              </ul>
                        </div>
                        <div id="similarItems"><b>You Might Also Like:</b>
                        		<br /><br />
                              <ul>
                                  <li><a href="22_935.php"><b>22-935</b> Captain's Bed Twin</a></li>
                                  <li><a href="22_950.php"><b>22-950</b> Jr. Captain's Bed</a></li>
                                  <li><a href="22_940.php"><b>22-940</b> Jr. Captain's Bed</a></li>
                                  <li><a href="91_74.php"><b>91-76</b> Play &amp; Study Twin</a></li>
                                  <li><a href="40_830.php"><b>40-830</b> Enterprise Twin Bed</a></li>
                                  <br />
                              </ul>
                      </div>
                      
                      <?php include 'CGs_links.php'; ?>
                      
                 </div>
               </div>             
            </div>  
       </div>

<?php include 'footer.php'; ?>

</body>
</html>        
          																																																																				