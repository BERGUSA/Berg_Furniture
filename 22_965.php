<?php include 'head.php'; ?>
<title>Berg Furniture | Jr. Captain's Bed | 22&ndash;965</title>

</head>
	<body>
    	<div id="outside_container">

			<?php include 'options.php'; ?>

				<?php include 'bss.php'; ?>
			<div id="tip"></div>
            <div class="container">
                
                <?php include 'header.php'; ?>
                
               <div class="prodContainer">
               		<div class="prodTitle"><style> .prodTitle p a:hover {color:#8be93b ;}</style>
                    	<p><a href="jr_captains_bed.php">The Jr. Captain's Bed Collection</a></p>
                    </div>
                 <div class="prodMain">
                        <div class="prodCont">
                        		<h2>High Frame Twin with Stairs</h2><h3>#22-965</h3>
                                <br />
                                <p>
                               The low-set, high-frame Jr. Captain's Bed makes the perfect first bed for any toddler.  It's easy to climb into, yet safe and comfortable enough not to fall out.  The 8 extra-deep drawers and additional side shelves provide early inspiration for structure and organization.  The optional staircase provides easier access and additional storage space.  Perfect for tots making the transition from crib to big kid's bed. Stairs can be installed on either the left or right side.
                                </p>
                                <br />
                                <br />
                                <h3>Features</h3>
                                <ul>
                                	<li>Twin Size Bed</li>
                                    <li>High Frame</li>
                                    <li>8 Drawers</li>
                                    <li>Side Bookshelf</li>
                                    <li><a href="#" rel="#BSS">Berg Signature Staircase</a></li>
                                </ul>
                                <div class="dimensions"><img src="img/dimbkg.png"/>
                                    <div style="top:21px; left: 5px;">38"H</div>
                                    <div style="top:40px; left: 40px;">42"D</div>
                                    <div style="top:77px; left: 70px;">96"L</div>
                                </div>
                                <br />
                               <a href="#" rel="#knobs">Click for Knob Options&nbsp;&gt;&gt;</a><br />
                                <a href="#" rel="#finishes">Click for Finish Options&nbsp;&gt;&gt;</a></br >
								<a href="#" rel="#accents">Click for Accent Options&nbsp;&gt;&gt;</a>
                        </div>
                   <div class="prodShow">
                   	 <div class="imgWrap">
                            	<img src="img/1px.gif" width="500" height="392" />
                                <div id="image_caption"></div>
                            </div>
                            <a class="prevPage browse left"></a>
                            	<div class="scrollable">
                                	<div class="items">
                                    	<img src="img/MainProdPics/22-965A_t.jpg" 
                                        title="Shown in White with <b><i>Optional</b> Orange Oslo Knobs</i>"/>
                                        
                                        <img src="img/MainProdPics/22-965B_t.jpg" 
                                        title="Shown in White with Pastel Blue Knobs"/>
                                        
                                        <img src="img/MainProdPics/22-965C_t.jpg" 
                                        title="Shown in White with <b><i>Optional</b> Pastel Pink Oslo Knobs</i>"/>
                                        
                                        <img src="img/MainProdPics/22-965D_t.jpg" 
                                        title="Shown in White with <b><i>Optional</b> Orange Oslo Knobs</i>"/>
                                      
                                        <img src="img/MainProdPics/girl-mom-stairs_t.jpg" 
                                        title="Berg's Signature Staircase is comfortable for parents too!"/>
                                       
                                    </div>
                                </div>
                            <a class="nextPage browse right"></a>
                     </div>
                         
                      <div id="iconBar">
                            
                         <div id="icons">
                           	 <img src="img/images/icons/1.png" 
                                	title="<h4>One Bed</h4><br />This Smart Bedroom System is designed to provide an enriching environment for one child" />
                              
                             
                             
                             <img src="img/images/icons/twin.png" 
                                title="<h4>Twin Bed</h4><br />The frame on this furniture set is built to support a twin 	
                                			 sized bed. Mattress not included." />
                            
                             <img src="img/images/icons/stairs.png" 
                                	title="<h4>Berg's Signature Staircase</h4><br />This system is equipped with Berg's Signature Staircase. This innovative 				 											original feature provides safe access to the top loft and lots of storage in each step" />
                                    <div id="rollover">Roll-over icons for more info</div>
                         </div>
                     </div>        
                   </div>
                    <div class="prodFtr">
                    	<style> #thisColl a:hover {color:#8be93b;}</style>
                        <div id="thisColl"><b><a href="/jr_captains_bed.php">All Jr. Captain's Beds</a></b>
                        		<br /><br />
                              <ul>
                               	  <li><a href="22_940.php"><b>22-940</b> High Frame Twin</a></li>
                                  <li><a href="22_950.php"><b>22-950</b> Low Frame Twin</a></li>
                                  <li><a href="22_952.php"><b>22-952</b> Low Frame Full 16 Drawer</a></li>
                                  <li><a href="22_959.php"><b>22-959</b> Low Frame Full</a></li>
                                  <li style="display:block; background-color: #8be93b;color:#000;"><b>22-965</b> High Frame Twin w/Stairs</li>
                                  
                                  <br />
                              </ul>
                        </div>
                        <div id="similarItems"><b>You Might Also Like:</b>
                        		<br /><br />
                              <ul>
                                  <li><a href="22_959.php"><b>22-940</b> Jr. Captain's Twin</a></li>
                                  <li><a href="22_931.php"><b>22-895</b> Captain's Bed Full</a></li>
                                  <li><a href="22_935.php"><b>22-935</b> Captain's Bed Twin</a></li>
                                  <li><a href="91_74.php"><b>91-74</b> Play &amp; Study</a></li>
                                  <li><a href="40_830.php"><b>40-830</b> Enterprise Twin</a></li>
                                  
                              </ul>
                      </div>
                      
                      <?php include 'CGs_links.php'; ?>
                      
                 </div>
               </div>             
            </div>  
       </div>
   	
    <?php include 'footer.php'; ?>
    
</body>
</html>        
          																																																																				