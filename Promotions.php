<?php include 'head.php'; ?>
<title>Berg Furniture | Promotions</title>
</head>

<script type="text/javascript" src="Berg_Furniture_Online/js/jquery-1.3.2.min.js"></script>
<script type="text/javascript" src="Berg_Furniture_Online/js/jquery.tools.js"></script>
<script type="text/javascript" charset="utf-8">


	  
	  $(document).ready(function(){
			  
 	 
		
										   
		$("div.tabs").tabs(".FF_Images > div", { 
 
        // enable "cross-fading" effect 
        effect: 'fade', 
        fadeInSpeed: "slow",
		fadeOutSpeed: "slow", 
 
        // start from the beginning after the last tab 
        rotate: true 
 
    // use the slideshow plugin. It accepts its own configuration 
    }).slideshow({
		autoplay: true,
		interval: 5000
		
	
	
	});



}); 


</script>

<script type="text/javascript">
var gaJsHost = (("https:" == document.location.protocol) ? "https://ssl." : "http://www.");
document.write(unescape("%3Cscript src='" + gaJsHost + "google-analytics.com/ga.js' type='text/javascript'%3E%3C/script%3E"));
</script>

<script type="text/javascript">
try {
var pageTracker = _gat._getTracker("UA-10894475-1");
pageTracker._trackPageview();
} catch(err) {}
</script>

   <style> 
  
  input[type="button"] {
	  padding: 15px; font-size:20px; 
	  font-weight:bold; cursor:pointer;  
	  -moz-border-radius: 10px; 
	  -webkit-border-radius: 10px; 
	  background: #7EEDDA;
	  color: #23A993; 
	  text-shadow: 1px 1px 0px #CCF5EE;
	  border: none;
	  -moz-box-shadow: 1px 3px 1px #5EDEC9;
	  -webkit-box-shadow: 1px 3px 1px #5EDEC9;
	   box-shadow: 1px 3px 1px #5EDEC9; 
  }
  
  input[type="button"]:hover {
	background: #A5F3E6;
  } 
  
  @media print {
  
	input, #header, #footer {
		display:none;
	}
 } 
  
  </style>
 
</head>
	<body> 
    	<div id="outside_container">
            
           
            <div class="container">
                
                
               <div class="prodContainer">
               		<div class="prodTitle">
                    </div>
                 	<div class="prodMain">
                    	<div style="margin:20px;">                      
							<input type="button" 
										onClick="pageTracker._trackEvent('Coupons', 'Print', 'Free Shelf'); window.print();"  
										value="Click Here to Print out this Coupon!"
							/>
						</div>
						<img src="img/Shelf-Coupon_1-2012.jpg" alt="Berg Furniture Free Shelf Coupon Now to February 5th Promotion" title="Berg Furniture Free Shelf Coupon!!!" />
                        <div style="margin:20px;">  
                        	
                        	                    
							<input type="button" 
										onClick="pageTracker._trackEvent('Coupons', 'Print', 'Free Shelf'); window.print();"  
										value="Click Here to Print out this Coupon!" 
							/>
						</div> 
					</div>
				</div>        
            </div>
                    
        </div>             
       
       
   
   <!--#include file="footer_new.asp" -->
   
</body>
</html>        
          																																																																				